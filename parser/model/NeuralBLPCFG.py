...
class NeuralBLPCFG(nn.Module):
    def __init__(self, args):
        ...
        self.head_mlp = nn.Sequential(
            ResLayer(self.s_dim, self.s_dim), nn.Linear(self.s_dim, self.r)
        )
    ...
    def get_grammar(self, input):
        ...
        noninherent_symbol = (
            self.noninherent_mlp(self.r_emb2)
            .log_softmax(-1)
            .reshape(self.r, self.NT_T, 2)
            .transpose(1, 0)
            .unsqueeze(0)
            .expand(b, -1, -1, -1)
        )
        ...
        noninherent = noninherent_word.unsqueeze(-2).unsqueeze(
            -1
        ) + noninherent_symbol.unsqueeze(1)
    ...
    def forward4viterbi(self, input):
        ...
        noninherent_word = torch.gather(
            noninherent_word, 1, x.unsqueeze(-1).expand(-1, -1, self.r)
        )
        ...
        def roots():
            ...
            roots = self.root_mlp(root_emb).log_softmax(-1)
            roots_v = self.root_mlp2(self.nonterm_emb).log_softmax(-1)
            roots_v = torch.gather(
                roots_v.unsqueeze(0).expand(b, self.NT, self.V),
                -1,
                x.unsqueeze(1).expand(-1, self.NT, -1),
            )
            return roots.expand(b, self.NT).unsqueeze(1) + roots_v.transpose(
                -1, -2
            )
