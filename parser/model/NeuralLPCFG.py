...
class NeuralLPCFG(nn.Module):
    def __init__(self, args, dataset):
        ...
        self.nonterm_emission_emb = nn.Parameter(
            torch.randn(self.NT, self.s_dim)
        )
        ...
        self.left_rule_mlp = nn.Linear(
            self.s_dim + self.s_dim, (self.NT_T) ** 2
        )
        self.right_rule_mlp = nn.Linear(
            self.s_dim + self.s_dim, (self.NT_T) ** 2
        )
    ...
    def get_grammar(self, input, eval_dep=False, **kwargs):
        ...
        def terms():
            ...
            term_prob = self.term_mlp(
                torch.cat([self.nonterm_emission_emb, term_emb], dim=0)
            ).log_softmax(-1)
            indices = (
                x.unsqueeze(2).expand(b, n, self.NT + self.T).unsqueeze(3)
            )
            term_prob = torch.gather(
                term_prob.unsqueeze(0)
                .unsqueeze(0)
                .expand(b, n, *term_prob.shape),
                3,
                indices,
            ).squeeze(3)
            return term_prob
        ...
        def rules():
            ...
            nt_x_emb = torch.cat(
                [
                    x_emb.unsqueeze(2).expand(-1, -1, self.NT, -1),
                    nt_emb.unsqueeze(1).expand(-1, n, -1, -1),
                ],
                dim=3,
            )
            left_rule_score = self.left_rule_mlp(nt_x_emb)  # nt x t**2
            right_rule_score = self.right_rule_mlp(nt_x_emb)  # nt x t**2
            left_rule_scores = left_rule_score.reshape(
                b, n, self.NT, self.NT_T, self.NT_T
            )
            right_rule_scores = right_rule_score.reshape(
                b, n, self.NT, self.NT_T, self.NT_T
            )
        ...
        if eval_dep:
            left = (
                torch.einsum(
                    "qnabc, qmc -> qmnabc", rule[:, 0].exp(), unary.exp()
                )
                .add_(1e-20)
                .log()
            )
            right = (
                torch.einsum(
                    "qnabc, qmb -> qmnabc", rule[:, 1].exp(), unary.exp()
                )
                .add_(1e-20)
                .log()
            )
            ...
            rule = left.mul_(
                torch.tril(
                    torch.ones(n, n, device=torch.device("cuda")), diagonal=-1
                )
                .unsqueeze(0)
                .unsqueeze(-1)
                .unsqueeze(-1)
    ...
    def forward(self, input):
        ...
        # Partition function
        if self.depth > 0:
            self.pf = self.pcfg._partition_function(
                rules=rules, depth=self.depth
            )
    ...
    def evaluate(self, input, decode_type="mbr", eval_dep=False):
        ...
        if self.depth > 0:
            ...
            for d in range(self.depth + 1):
                p = (
                    self.pcfg._partition_function(rules=rules, depth=d)
                    .unsqueeze(1)
                    .exp()
                )
                if d == 0:
