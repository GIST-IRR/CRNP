...
class TDPCFG(PCFG_base):
    @torch.enable_grad()
    def _inside(self, rules, lens, mbr=False, viterbi=False, label=False):
        ...
        if not mbr and not viterbi:
            ...
            return {
                "prediction": self._get_prediction(
                    logZ, span_indicator, lens, mbr=True, label=label
                ),
                "partition": logZ,
            }
...
class Fastest_TDPCFG(PCFG_base):
    ...
    def _inside(
        ...
        if s_span is None:
            ...
            if mbr or viterbi:
                span_indicator = (
                    span_indicator.detach().clone().requires_grad_(True)
                )
